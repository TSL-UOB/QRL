...
def energyField_update(nA,nExp,agentState,indexID,indexIDbool,Ef_Z,gridH,Ef_Log,diag):
	...
	for agentID in range(0,nA):
		if indexIDbool[agentID]: #which agent generated the test
			...
			# Ef_Z[valid_x,valid_y] -= 1 #reduce energy for good tests
			# if(diag):
			if(diag):print("Test generated from (numpy coords) x=%d y=%d" % (valid_x,valid_y) )
...
def energyField_plot(nExp,Ef_X, Ef_Y,Ef_Z,nTests,agentBehaviour,display_chart_modulo,SaveCharts,camera,plotgrid_x,plotgrid_y,grid_x=0,grid_y=0,gifAnimation=False):
	...
	# if EF_spawn:
	if (np.shape(plotgrid_x)[0]>0) and Ef_start:
...
nTests = 1001					# Number of experiements to running_score
...
delay = 0.05					# delay between each frame, slows sim down
...
plotContour	  = False			# plot a contour of the energy field
plot3DContour = False			# plot a 3D contour of the energy field
SaveCharts    = False 			# save the plots produced
gifAnimation  = False 			# save a gif of the plot sequence
...
if loopAgentList:
	...
	agList = [0,1,2,3,4] # Loop agent types
	vtList = [50,100,200,500,700,800,900,1000,1100,1200,2000,5000,10000,20000,50000,100000,200000,500000]
	epList = [0.005,0.01,0.02,0.04,0.06,0.08,0.1,0.12,0.14,0.16,0.18,0.20,.22,.24]
else:
	nAList = [nA]
if plotFeatures:
	fig = plt.figure()
...
# for epsilon in epList:
	if loopAgentList: agentBehaviour = agentChoices[ag]
	   ...
	   #  			if (SaveCharts) and plot3DContour and (nExp%display_chart_modulo==0):	
			...
			if done:
				...
				if (nExp>nTests-1):
					...
					# if Ef_start and (epsilon > np.random.uniform(0.0, 1.0)): #could change this to < or have increasing function
					if Ef_start and (epsilon < np.random.uniform(0.0, 1.0)): #could change this to < or have increasing function
	...
	ci95= list(zip(*agScore_95ci))
	ci95Arr = np.asarray(ci95)
	# print("agScore_AVG %.2f agScore_MAX %.2f" % (agScore_AVG, agScore_MAX))
	# print("agScore_MIN " ,agScore_MIN)
	# print("agScore_95ci " ,agScore_95ci)
	# print("ci95Arr" , ci95Arr )
